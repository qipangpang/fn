 ...
 * 1.结构:JSON对象{}/JSON数组[]
 * 2.数据: 
 * json对象数据 {key1:value1,key2:value}
 * json数组数据 [value1,value2,value3]
 * 数据中: key:"String"-----字符串类型
 * 数据中: value:String,Number,Boolean,对象{},[]
 * json和js是什么关系? json是js中一种特殊格式数据的字符串
 * JSON.parse()
 * JSON.stringfy()
 * 
 * 通过JSON设计数据
 ...
 * 拦截ajax请求,随机生成数据
 * 2.多模块开发----拆分store
 * state中 有msite相关的数据状态-----user相关的数据状态---shop相关数据的状态
 * mutations 中------
 * actions 中-------
 * 
 * 代码冗余,不方便维护,臃肿,.....
 * 拆分,多模块编程
 * msite.js----state,mutations,actions,getter
 * user.js
 * shop
 * modules中---
 * 
 * store目录中index.js中引入msite.js模块,引入user.js模块,引入shop.js模块
 * 
 * (store目录中)index.js
 ...
 *  getters,     //
 *  modules:{
 *  msite,------------模块的名字(中有state---categorys数据) 外部...mapState({categorys:state=>state.msite.categorys})actions中也有xxx方法
 *  user,
 *  shop
 *  
 *  }
 * })
 * 
 * 如果在外部dispatch('xxx')--总actions中的xxx和msite中的actions中的xxx虽然同名.但是都会被调用,顺序:总,分
 * 
 * 
 * 
 */
